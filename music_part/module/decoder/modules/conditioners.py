...
class CLIPConditioner(VideoConditioner):
    ...
    def __init__(self, name: str, output_dim: int, device: str, video_len: int = 30, finetune: bool = False, is_qformer: bool = False, qformer: omegaconf.DictConfig = None):
        ...
        #     nn.Conv1d(96, 96, kernel_size=2, stride=2, padding=0),
        #     nn.ReLU(),
        #     nn.Conv1d(96, 96, kernel_size=2, stride=2, padding=0),
        #     nn.ReLU(),
        #     nn.Conv1d(96, 96, kernel_size=2, stride=2, padding=0),
    ...
    def tokenize(self, x: tp.List[tp.Optional[str]]) -> tp.Dict[str, torch.Tensor]:
        ...
        for v in entries:
            if v != "":
                ...
                # video = video.reshape(video.shape[0], -1, 768)
                # video = torch.mean(video, dim=1)
                # video = video.permute(0, 2, 1)  # [T, 64, 96] -> [T, 96, 64]
                # video = self.conv_net(video)    # [T, 96, 8]
                # video = video.permute(0, 2, 1)  # [T, 8, 96]
                # video = video.reshape(video.size(0), -1)
                videos["video"].append(video)
                videos['attention_mask'].append(1)
            else:
                if self.training:
                    video = torch.zeros(self.video_len, 768).to(self.device)
                else:
                    video = torch.zeros(videos["video"][0].shape[0], 768).to(self.device)
                videos["video"].append(video)
