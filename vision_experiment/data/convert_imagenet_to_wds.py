...
def convert_imagenet_to_wds(output_dir, max_train_samples_per_shard, max_val_samples_per_shard):
    ...
    dataset = load_dataset("imagenet-1k", streaming=True, split="train", cache_dir="/cache")
    now = time.time()
    ...
    dataset = load_dataset("imagenet-1k", streaming=True, split="validation", cache_dir="/cache")
    now = time.time()
    for i, example in enumerate(dataset):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, required=True, help="Path to the output directory.") # /archive/public_data/imagenet
